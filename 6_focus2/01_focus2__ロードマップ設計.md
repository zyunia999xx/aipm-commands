  ...
  steps:
    ...
    - name: "show_inputs"
      ...
      content: |
        ...
        - 4_delivery: 07_実装__開発タスク分解/dev_tasks.yaml
    ...
    - name: "notify"
      ...
      message: |
        ...
        次は `4_delivery/07_Delivery__開発タスク分解` にテーマ/フィーチャを反映して実装計画へ進みましょう。
```

## 次に実行
- `4_delivery/07_Delivery__開発タスク分解`（テーマ→フィーチャ→STへの展開）
- 必要に応じて `01_strategy__プロダクトメトリクス` で測定設計を更新
