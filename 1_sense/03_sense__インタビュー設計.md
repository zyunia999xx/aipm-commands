...
- trigger: "(sense_インタビュー設計|インタビュー質問|質問表作成|インタビューガイド)"
  ...
  steps:
    ...
    - name: "ensure_output_dir"
      ...
      command: "mkdir -p {{patterns.flow_date}}/1_sense/03_sense__インタビュー設計"

    - name: "create_interview_guide"
      action: "create_markdown_file"
      path: "{{patterns.flow_date}}/1_sense/03_sense__インタビュー設計/draft_interview_guide.md"
      content: |
        # インタビューガイド
        - 目的: {{iv.research_goal}}
    ...
    - name: "notify"
      ...
      message: |
        ...
        次は「05_sense__リクルーティング計画」へ。
```

## 次に実行
- `05_sense__リクルーティング計画`
