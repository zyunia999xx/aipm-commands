...
- trigger: "(sense_仮想インタビュー|virtual_interview|SynthInterview)"
  ...
  steps:
    - name: "infer_context"
      action: "analyze"
      data: ["{{thread_messages}}", "{{find_files(patterns=['**/1_sense/03_sense__インタビュー設計/draft_interview_guide.md','**/problem_*.md','**/solution_*.md','**/persona_*.md'])}}"]
      instructions: |
    ...
    - name: "ensure_output_dir"
      ...
      command: "mkdir -p {{patterns.flow_date}}/1_sense/90_sense__仮想インタビューログ生成"

    - name: "generate_virtual_logs"
      action: "create_markdown_file"
      path: "{{patterns.flow_date}}/1_sense/90_sense__仮想インタビューログ生成/virtual_interview_logs.md"
      content: |
        # 仮想インタビューログ（発散用）
        - 目的: {{seed.3}}
        - 対象: {{seed.1}}
    ...
    - name: "notify"
      ...
      message: |
        ✅ 仮想インタビューログを作成しました：
        {{patterns.flow_date}}/1_sense/90_sense__仮想インタビューログ生成/virtual_interview_logs.md
```
